...
class RasterEdit(RasterManager):
    ...
    def rect(self, fin, fout, bounds, points, mode='asc', hop=2, carve=-1):
            ...
            for count in range(len(pd)):
                ...
                for x, y, dx, dy in surround_point(px,py,hop=hop):
                    ...
                    if y >=0 and y < empty.shape[1] and x >=0 and x < empty.shape[2]:
                        # if dx == 3 and dy == 3:
                        #     empty[0][y,x] =  data[0][y,x] - (data[0][y,x]-elev)/2.0     

                        # elif dx == 2 and dy == 2:
                        #    empty[0][y,x] =  data[0][y,x] - (data[0][y,x]-elev)/3.0     

                        # elif dx == 1 and dy == 1:
                        #     empty[0][y,x] =  data[0][y,x] - (data[0][y,x]-elev)/5.0

                        #if dx == 0 and dy == 0:
                        #    empty[0][y,x] =  elev # the elevation of the track
                        empty[0][y,x] = data[0][y,x]
...
if __name__ == "__main__":
    ...
    points,gpx_bounds,distance = smooth_gpx(args.gpxfile,
                ...
                geoid=args.geoid,
                output=None)
    rasteredit.rect(args.infile, args.outfile, bounds, points, hop=args.width, carve=args.carve)
